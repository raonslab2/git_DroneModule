...
async def run(droneInfo,*args):
    ...
    asyncio.ensure_future(print_serversend(droneInfo,conn,curs))
...
def getConnection():
    ...
    try:        
        con = db.connect(host='34.64.179.209', 
                         user='samsungdrone', 
                         password='Min002004!@#',
                         db='sepm_db', 
                         charset='utf8',
...
async def print_serversend(droneInfo,conn,curs):
    running = True
    tmName = droneHost; 
    ...
    while running:     
        ...
        try:
            ...
            print('Connectind to vehicle on: %s' % connection_string)
            #global drone
            #drone = connect(connection_string)
            
            
            st_battery = 0.0
            if isNumber(drone.battery.voltage) == True:
                st_battery = drone.battery.voltage
            sql = "INSERT INTO br_drone_state"+ \
                "(dl_id, st_status, st_satelite_num, st_bat_voltage, st_bat_level, st_speed, st_x, st_y, st_z, st_atitude,st_roll,st_pitch,st_yaw,st_head,st_state,st_mode,st_arm)" + \
                "VALUES('" + tmName + "', " + \
                "1," + \
            ...
            try:
                #drone.close()
                print("finally NameError")
            except NameError:
                print("finally NameError")
            except:
                print("finally NameError2") 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        host = "ws://34.64.179.209:5010/websocket"
